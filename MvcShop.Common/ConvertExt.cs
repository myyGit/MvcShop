...
namespace Common
{
    public static class ConvertExt
    {
        ...
        /// <returns></returns>
        public static T O2D<T, T1>(this T SourceT, T1 TragetT, bool IsConvertNull = false, List<KeyValuePair<string, string>> ConvertRule = null)
        {
            if (SourceT == null)
            {
        ...
        /// <param name="ConvertRule">特殊转化规则key为源属性，value为目标属性</param>
        /// <returns></returns>
        public static List<T> O2D<T, T1>(this List<T> SourceList, List<T1> TragetList, bool IsConvertNull = false, List<KeyValuePair<string, string>> ConvertRule = null)
        {
            if (TragetList != null)
            {
                if (SourceList == null)
                ...
                for (int i = 0; i < TragetList.Count(); i++)
                {
                    ...
                    SourceList[i] = SourceList[i].O2D(TragetList[i], IsConvertNull, ConvertRule);
                }
            }
            return SourceList;
        }
